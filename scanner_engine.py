# scanner_engine.py (Final Polished Version 1.7 - Kali Linux)
...
# --- CONFIGURATION ---
# In Kali Linux, Nikto is already installed and in the system's PATH.
# We no longer need a path to the script, just the command.
...
# --- THE FINAL, TRULY CORRECTED NIKTO FUNCTION for KALI ---
def run_nikto_scan(domain: str):
    """Runs a comprehensive Nikto scan and captures all output correctly."""
    print(f"\n[+] Starting Comprehensive Nikto scan for: {domain}")
    # The command is now simple because Nikto is installed system-wide in Kali.
    command = ["nikto", "-h", domain, "-Tuning", "4"]
    
    try:
        # We run the process and explicitly capture stdout and stderr.
        # check=False is important, as Nikto can exit with a non-zero code on findings.
        ...
        return "[!] ERROR: 'nikto' command not found. This should not happen on a standard Kali install."
